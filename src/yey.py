...
def parse_input(input_str, iterations_str):
    try:
        ...
        control_points = [tuple(map(float, point.strip("()").split(","))) for point in control_points_str]
...
def plot_bezier_curve(method):
    ...
    if control_points is None or iterations is None:
        print("Invalid input. Please check the format of the control points and iterations.")
        return
...
frame_inputs = ttk.Frame(root, padding="10", relief=tk.RIDGE, borderwidth=2, style='Custom.TFrame')
...
label_control_points = ttk.Label(frame_inputs, text='Control Points(x,y),(x,y),.. :', background=frame_color, foreground=text_color)
entry_control_points = ttk.Entry(frame_inputs, width=40)
label_iterations = ttk.Label(frame_inputs, text='Iterations:', background=frame_color, foreground=text_color)
...
radio_brute = ttk.Radiobutton(frame_inputs, text='Brute Force', variable=method_var, value='brute')
radio_dnc = ttk.Radiobutton(frame_inputs, text='Divide and Conquer', variable=method_var, value='dnc')
radio_brute.grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)
radio_dnc.grid(row=2, column=1, padx=5, pady=5, sticky=tk.W)

# Plot button
button_submit_bezier = ttk.Button(frame_inputs, text='Plot Bezier Curve', command=lambda: plot_bezier_curve(method_var.get()))
button_submit_bezier.grid(row=3, column=0, columnspan=2, pady=5, sticky=tk.W+tk.E)

style.configure('TButton', background=button_color, foreground=text_color, padding=5)
